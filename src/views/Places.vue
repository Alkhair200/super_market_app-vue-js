            ...
            <div class="row breadcrumbs-top d-inline-block">
              <div class="breadcrumb-wrapper col-12">
                <ol class="breadcrumb">
                  ...
                  <li class="breadcrumb-item"><a href="#">إضافه موقع</a></li>
                </ol>
              </div>
            </div>
          </div>
            ...
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  <h4 class="modal-title" id="myModalLabel69">إضافه موقع</h4>
                  <button
                    type="button"
                    class="close"
                    data-dismiss="modal"
...
export default {
  ...
  computed: {
    ...
    findData() {
      return this.allplaces.filter((place) => {
        return place.name.match(this.dataSearch);
      });
    },
  },
  ...
  methods: {
    ...
    store() {
      this.places.token = this.getToken;
      this.axios
        .post("http://127.0.0.1:8000/api/place/store", this.places)
        .then((res) => {
          if (res.data.status == true) {
            this.$notify({
              title: "عمليه ناجه 🎉",
    ...
    addNew() {
      this.edit_place = false;
      this.places = {};
      this.errors = [];
    },

    edit(item) {
      this.edit_place = true;
      this.places = item;
      this.errors = [];
    },
    ...
    update() {
      ...
      this.axios
        .patch("http://127.0.0.1:8000/api/place/update/" + this.places.id, form)
        .then((res) => {
          if (res.data.status == true) {
            this.$notify({
              title: "عمليه ناجه 🎉",
    ...
    deleteItem(id) {
      this.$swal({
        ...
        if (result.isConfirmed) {
          this.axios
            .post("http://127.0.0.1:8000/api/place/destroy/" + id, {
              token: this.getToken,
            })
            .then((res) => {
              if (res.data.status == true) {
