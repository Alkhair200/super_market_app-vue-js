            ...
            <div class="row breadcrumbs-top d-inline-block">
              <div class="breadcrumb-wrapper col-12">
                <ol class="breadcrumb">
                  ...
                  <li class="breadcrumb-item"><a href="#">إضافه عميل</a></li>
                </ol>
              </div>
            </div>
          </div>
            ...
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  <h4 class="modal-title" id="myModalLabel69">إضافه عميل</h4>
                  <button
                    type="button"
                    class="close"
                    data-dismiss="modal"
...
export default {
  ...
  computed: {
    ...
    findData() {
      return this.allcustomers.filter((customer) => {
        return customer.name.match(this.dataSearch);
      });
    },
  },
  ...
  methods: {
    getcustomers() {
      this.isLoading = true;
      this.axios
        .post("http://127.0.0.1:8000/api/customer/index", {
          token: this.getToken,
        })
        .then((res) => {
          this.isLoading = false;
          if (res.data.status == true) {
            this.allcustomers = res.data.customer;
    ...
    storecustomer() {
      this.customers.token = this.getToken;
      this.axios
        .post("http://127.0.0.1:8000/api/customer/store", this.customers)
        .then((res) => {
          if (res.data.status == true) {
            this.$notify({
              title: "عمليه ناجه 🎉",
    ...
    updatecustomer() {
      ...
      this.axios
        .patch(
          "http://127.0.0.1:8000/api/customer/update/" + this.customers.id,
          form
        )
        .then((res) => {
          if (res.data.status == true) {
    ...
    deleteItem(id) {
      this.$swal({
        ...
        if (result.isConfirmed) {
          this.axios
            .post("http://127.0.0.1:8000/api/customer/destroy/" + id, {
              token: this.getToken,
            })
            .then((res) => {
              if (res.data.status == true) {
