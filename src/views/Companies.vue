            ...
            <div class="row breadcrumbs-top d-inline-block">
              <div class="breadcrumb-wrapper col-12">
                <ol class="breadcrumb">
                  ...
                  <li class="breadcrumb-item"><a href="#">إضافه شركة</a></li>
                </ol>
              </div>
            </div>
          </div>
            ...
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  <h4 class="modal-title" id="myModalLabel69">إضافه شركة</h4>
                  <button
                    type="button"
                    class="close"
                    data-dismiss="modal"
...
export default {
  ...
  computed: {
    ...
    findData() {
      return this.allcompanies.filter((company) => {
        return company.name.match(this.dataSearch);
      });
    },
  },
  ...
  methods: {
    ...
    store() {
      this.companies.token = this.getToken;
      this.axios
        .post("http://127.0.0.1:8000/api/company/store", this.companies)
        .then((res) => {
          if (res.data.status == true) {
            this.$notify({
              title: "عمليه ناجه 🎉",
    ...
    update() {
      ...
      this.axios
        .patch(
          "http://127.0.0.1:8000/api/company/update/" + this.companies.id,
          form
        )
        .then((res) => {
          if (res.data.status == true) {
    ...
    deleteItem(id) {
      this.$swal({
        ...
        if (result.isConfirmed) {
          this.axios
            .post("http://127.0.0.1:8000/api/company/destroy/" + id, {
              token: this.getToken,
            })
            .then((res) => {
              if (res.data.status == true) {
